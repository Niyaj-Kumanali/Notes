...
- **Feature envy** — A method in class A that calls five getters on class B to do its calculation. This starts when a method needs data from another class to compute something, and the path of least resistance is to call getters rather than move the logic.
  - **Why it looks correct:** The method still works and returns the right result. Over time, class B's interface becomes a procedural data bag with getters for everything, and class A accumulates business logic that should belong to B. When the data structure changes, every envious method across the codebase must be updated — a single field rename becomes a week-long refactoring project.
