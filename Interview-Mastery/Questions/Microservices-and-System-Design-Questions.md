...
1. What is microservices architecture?
   - **Answer:**
      ...
      - The cold-chain system could be refactored into microservices: a sensor-ingestion service writing to InfluxDB for time-series data, an alert-engine service evaluating temperature thresholds, and a dashboard-api service querying PostgreSQL for configuration, each with its own database and deployable independently
2. Difference between monolith and microservices.
   - **Answer:**
...
3. Advantages of microservices.
   - **Answer:**
      ...
      - Microservices improve fault isolation - one service failing does not bring down the whole system, critical for cold-chain where alerting must stay up even if the dashboard has issues
4. Disadvantages of microservices.
   - **Answer:**
...
22. What is event-driven architecture?
    - **Answer:**
      ...
      - **Event sourcing** stores every state change as an immutable event in a log (Kafka topics), making the system fully replayable and auditable - you can rebuild state by replaying events from the beginning. **Event notification** is simpler: services publish events to signal something happened, but the event is not the source of truth. Kafka's log-based approach gave us event sourcing benefits naturally - we could replay sensor data to reprocess historical periods or debug data pipeline issues
23. What is idempotency?
    - **Answer:**
...
39. How do you handle data consistency?
    - **Answer:**
      ...
      - CAP theorem tradeoffs: in distributed systems, during a network partition, you choose between consistency (CP - reject writes until partition heals) and availability (AP - accept writes and resolve conflicts later). For our cold-chain system, availability was paramount - sensor data must always be ingested, even if some services are temporarily unreachable, so we chose AP with eventual consistency. Design APIs to handle this: return stale data with a timestamp, use version vectors for conflict detection, and implement compensating actions for business-critical operations
40. How do you handle failure in downstream services?
    - **Answer:**
