...
1. What is JPA?
   - **Answer:**
      ...
      - JPA entities are mapped to database tables using `@Entity`, `@Table`, and `@Column` annotations to interact with data without writing manual SQL
...
2. What is Hibernate?
   - **Answer:**
      ...
      - Hibernate acts as the underlying engine for Spring Data JPA — it generates SQL queries, manages the first-level cache, and handles entity state transitions automatically
...
3. Difference between JPA and Hibernate.
   - **Answer:**
      ...
      - JPA annotations and interfaces like `EntityManager` and `@Entity` keep code portable, while Hibernate runs underneath
      - Hibernate-specific features should be used sparingly to avoid vendor lock-in
...
4. What is entity?
   - **Answer:**
      ...
      - Entity classes have fields annotated with `@Column` to define column names, lengths, and nullability in the database
...
5. What is `@Id`?
   - **Answer:**
      ...
      - Typically used with `@GeneratedValue(strategy = GenerationType.IDENTITY)` to let the database auto-increment the primary key
      - Composite primary keys use `@IdClass` (defining a separate POJO key class) or `@EmbeddedId` (embedding an `@Embeddable` key class); `@EmbeddedId` can be used with a composite key containing multiple fields to uniquely identify records
6. What is generated value?
   - **Answer:**
      - `@GeneratedValue` specifies how the primary key is auto-generated
      - `GenerationType.IDENTITY` is suitable for databases that support auto-increment columns and is simpler and more efficient than `SEQUENCE` for single-record operations
      ...
      - IDENTITY is often chosen despite the batch insert limitation because inserts are single-record operations where the overhead of configuring a sequence is not justified
7. What is repository?
   - **Answer:**
      - A repository is a Spring Data interface that provides CRUD operations without implementation code
      - Defining `InventoryRepository extends JpaRepository<InventoryRecord, Long>` automatically provides methods like `findAll()`, `save()`, `deleteById()`, and derived query methods
...
8. What is Spring Data JPA?
   - **Answer:**
      ...
      - Defining `PartnerRepository extends JpaRepository` automatically provides implementations for CRUD, pagination, and derived queries without writing any DAO code
      ...
      - `@Query` annotations are used for complex queries that cannot be expressed as derived methods, such as multi-table joins or database-specific logic
9. What is `JpaRepository`?
   - **Answer:**
      ...
      - `JpaRepository` is suitable for entities that need pagination and batch operations
...
10. Difference between `CrudRepository` and `JpaRepository`.
    - **Answer:**
       ...
       - `JpaRepository` is commonly used when pagination is needed for listing APIs
...
11. What is derived query method?
    - **Answer:**
       ...
       - Defining `findByPartnerIdAndStatus(Long partnerId, String status)` on a repository causes Spring Data JPA to automatically generate the JPQL query based on the method name — no need to write queries manually
...
12. What is JPQL?
    - **Answer:**
       ...
       - JPQL queries like `SELECT p FROM PartnerEntity p WHERE p.status = :status` query entities without database-specific SQL
...
13. Difference between JPQL and native query.
    - **Answer:**
       ...
       - JPQL is used for standard queries while native queries with `@Query(value = "EXEC sp_generate_report :partnerId", nativeQuery = true)` are used for executing database-specific stored procedures
...
14. What is entity lifecycle?
    - **Answer:**
       ...
       - Entities retrieved via `findById()` are in the managed state — any changes to them are automatically persisted at flush time without calling `save()`
...
15. What is persistence context?
    - **Answer:**
       ...
       - When an entity is loaded inside a `@Transactional` service method, the persistence context keeps track of all field modifications and flushes them to the database at commit time
...
16. What is dirty checking?
    - **Answer:**
       ...
       - Loading an entity, updating a field inside a `@Transactional` method, and committing causes Hibernate to automatically generate an UPDATE query at flush time without explicitly calling `save()`
...
17. What is first-level cache?
    - **Answer:**
       ...
       - Loading the same entity twice by `findById()` inside the same transaction returns the cached object without a second database query, improving performance for repeated lookups
...
18. What is second-level cache?
    - **Answer:**
       ...
       - Hibernate's second-level cache can be configured with Redis as the cache store using `hibernate-cache-redis`, caching frequently accessed but rarely changed entity data like configuration records
...
19. What is lazy loading?
    - **Answer:**
       ...
       - An entity with a `@OneToMany(fetch = FetchType.LAZY)` collection will load the associated records from the database only when the collection is accessed
...
20. What is eager loading?
    - **Answer:**
       ...
       - Using `@ManyToOne(fetch = FetchType.EAGER)` on a reference field ensures the associated data is always loaded alongside the parent entity, avoiding the N+1 problem for that relationship
...
21. What is N+1 query problem?
    - **Answer:**
       ...
       - Calling `findAll()` on a repository followed by accessing a lazy collection in a loop triggers N separate queries — making the operation extremely slow
       ...
       - The N+1 problem is identified by seeing hundreds of SELECT queries in the logs when iterating over associated collections — the initial query returns N parent entities, and each parent's collection triggers an additional SELECT
22. How do you solve N+1 query problem?
    - **Answer:**
       - N+1 can be solved using `JOIN FETCH` in JPQL or `@EntityGraph`
       - Changing a query from `SELECT p FROM PartnerEntity p` to `SELECT p FROM PartnerEntity p JOIN FETCH p.orders` fetches parents and children in a single SQL query using an INNER JOIN
...
23. What is fetch join?
    - **Answer:**
       ...
       - Using `SELECT p FROM PartnerEntity p JOIN FETCH p.orders` loads parents along with their associated collections in one SQL query, completely avoiding the N+1 problem
...
24. What is entity graph?
    - **Answer:**
       ...
       - Defining `@NamedEntityGraph(name = "Partner.withOrders", attributeNodes = @NamedAttributeNode("orders"))` and using `@EntityGraph("Partner.withOrders")` on the repository method provides flexible fetch strategies
...
25. What is pagination?
    - **Answer:**
       ...
       - `Pageable` parameter in repository methods (e.g., `Page<PartnerEntity> findAll(Pageable pageable)`) enables paginated results
       - The API returns page number, size, total elements, and total pages, allowing the frontend to display paginated tables
...
26. What is sorting?
    - **Answer:**
       ...
       - Using `Sort.by("partnerName").ascending()` in the repository method or passing `Sort` through `Pageable` controls result ordering
       - For multi-field sorting, use `Sort.by(Order.asc("partnerName"), Order.desc("createdDate"))`
...
27. What is transaction management?
    - **Answer:**
       ...
       - Spring's declarative transaction management with `@Transactional` on service methods ensures atomicity — if one step fails, the entire operation is rolled back
...
28. What is `@Transactional`?
    - **Answer:**
       ...
       - Using it on a batch processing service ensures that if any record fails validation, the entire batch is rolled back to maintain data consistency
...
29. How does `@Transactional` work internally?
    - **Answer:**
       ...
       - This works via CGLIB proxy — which means self-invocation bypasses the proxy and the annotation is ignored
       ...
       - `DataSourceTransactionManager` manages transactions for plain JDBC; `JpaTransactionManager` integrates with JPA's `EntityManager` and persistence context — `JpaTransactionManager` is used for JPA entities to ensure the persistence context and database transactions are synchronized
30. What is transaction propagation?
    - **Answer:**
       - Transaction propagation defines how transactions behave when a transactional method calls another transactional method
       - `REQUIRED` (default) is used for most methods, meaning they join the existing transaction
       - `REQUIRES_NEW` is used for audit logging methods so the audit log is saved even if the main transaction rolls back
       ...
       - `REQUIRES_NEW` is commonly used for notification services so alerts are committed independently — if the main transaction rolls back, notifications already sent are preserved
31. What is transaction isolation?
    - **Answer:**
       - Transaction isolation defines how concurrent transactions interact
       - `@Transactional(isolation = Isolation.READ_COMMITTED)` is a common choice and matches the default isolation level for most databases, preventing dirty reads
       - `REPEATABLE_READ` can be used for calculation methods to prevent phantom reads during stock or data computation
...
32. What is rollback behavior?
    - **Answer:**
       ...
       - This can be customized with `rollbackFor = {DataAccessException.class, CustomValidationException.class}` to ensure certain checked exceptions also trigger rollback
...
33. Why does self-invocation break `@Transactional`?
    - **Answer:**
       ...
       - This commonly occurs when method A (annotated with `@Transactional`) calls method B (also `@Transactional`) in the same service — the transaction for B is ignored
...
34. What is optimistic locking?
    - **Answer:**
       ...
       - Adding `@Version` on an entity prevents concurrent updates — if two users try to update the same record, the second commit throws `OptimisticLockException`
...
35. What is pessimistic locking?
    - **Answer:**
       ...
       - Using `@Lock(LockModeType.PESSIMISTIC_WRITE)` on a repository method ensures no other transaction can modify the data during a critical calculation
...
36. What is `@Version`?
    - **Answer:**
       ...
       - Adding `@Version private Long version;` to an entity causes Hibernate to automatically increment the version on every update and check it before committing, throwing `OptimisticLockException` on conflict
...
37. What are cascade types?
    - **Answer:**
       ...
       - Using `cascade = CascadeType.PERSIST` on the `@OneToMany(mappedBy = "partner")` orders collection ensures saving a parent also saves its children without separate save calls
       ...
       - Using only `PERSIST` and `MERGE` on `@OneToMany` associations propagates saves without accidentally deleting unrelated child records
38. What is orphan removal?
    - **Answer:**
       - Orphan removal automatically deletes child entities when they are removed from the parent's collection
       - Using `orphanRemoval = true` on a `@OneToMany` mapping ensures that when a child is removed from the parent's collection, it is automatically deleted from the database
       ...
       - Both can be used: `orphanRemoval = true` for cleaning up removed items from collections, and `CascadeType.REMOVE` for propagating parent deletion to dependent children
39. Difference between `save()` and `saveAndFlush()`.
    - **Answer:**
       ...
       - `saveAndFlush()` is useful when the generated ID is needed immediately for subsequent operations like logging
...
40. What is batch insert?
    - **Answer:**
       ...
       - Configuring `spring.jpa.properties.hibernate.jdbc.batch_size=50` and `hibernate.order_inserts=true` so that inserting 500 records in a loop triggers only 10 batch inserts instead of 500 individual queries
       ...
       - `rewriteBatchedStatements=true` in the JDBC connection URL rewrites individual INSERT statements into multi-row `INSERT ... VALUES (...), (...)` syntax, significantly improving batch insert performance
       - Batch configuration was discovered to dramatically improve performance during data import features, where unbatched inserts were taking several minutes and batch configuration reduced the time to seconds
41. How do you improve JPA performance?
    - **Answer:**
       - JPA performance can be improved by enabling batch operations, using fetch joins to avoid N+1 queries, configuring appropriate fetch types, enabling Hibernate query cache for read-heavy data, and monitoring slow queries via `spring.jpa.properties.hibernate.generate_statistics=true`
...
42. When should you use JDBC instead of JPA?
    - **Answer:**
       ...
       - `JdbcTemplate` can be used for stored procedure execution because it is simpler and faster than mapping complex result sets to entities for the reporting use case
       ...
       - Nightly batch processes handling millions of records benefit from JDBC because JPA's persistence context would cause OutOfMemoryError — `JdbcTemplate` with `RowMapper` processes rows one at a time without loading all entities into memory
43. When should you use stored procedures?
    - **Answer:**
       ...
       - Report generation is often implemented as a stored procedure because it involves complex joins, aggregations, and conditional logic that is more efficient in SQL
       - Benefits: reduced network round-trips (logic executes in the database engine), better performance for set-based operations (the database optimizer can plan the entire operation), and access to database-specific features like window functions, CTEs, temp tables, and bulk operations
       - Trade-offs: stored procedures are harder to unit test (require database integration tests), create vendor lock-in (e.g., MSSQL T-SQL is not portable to PostgreSQL), and business logic becomes harder to refactor since it lives outside the application codebase
44. How do you call stored procedures from Spring?
    - **Answer:**
       - Stored procedures can be called using `@Procedure` annotation on a repository method, or `JdbcTemplate` for direct execution
       - Using `@Procedure(procedureName = "sp_generate_report")` on a method in a JPA repository and mapping the result to a DTO using a custom `ResultSetExtractor` is the simplest approach
       ...
       - `JdbcTemplate` is preferred for complex stored procedures that return multiple result sets or have output parameters, since `@Procedure` does not handle these scenarios well
45. How do you debug slow JPA queries?
    - **Answer:**
       - Enable Hibernate SQL logging with `spring.jpa.show-sql=true` and `spring.jpa.properties.hibernate.format_sql=true`, and statistics logging with `hibernate.generate_statistics=true`
       - This helps identify N+1 queries and missing indexes by analyzing the generated SQL in the logs
       ...
       - Copying the generated SQL into MSSQL Management Studio's execution plan analyzer helps identify table scans, missing indexes, expensive key lookups, and parameter sniffing issues
46. Why do we use the `@Transactional` annotation?
    - **Answer:**
       ...
       - Saving multiple related records in the same method should be wrapped in one transaction so a failure mid-way does not leave the data half-updated
...
48. What types of exceptions trigger transaction rollback with `@Transactional`?
    - **Answer:**
       ...
       - To make a checked exception roll back, add `rollbackFor = Exception.class` (or a specific exception class) on the annotation
       ...
       - Related attributes: `noRollbackFor` (exceptions that should not trigger rollback), `rollbackForClassName` / `noRollbackForClassName` (string-based matching); using `@Transactional(rollbackFor = Exception.class)` on a method that catches a checked exception and rethrows it guarantees the entire transaction aborted instead of silently committing
