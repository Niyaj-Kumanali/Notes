...
1. What is React?
   ...
   - **If asked more:**
      - Would discuss how React's virtual DOM improved dashboard performance by minimizing actual DOM updates
      - Critical for rendering frequently updating sensor data without UI jank
2. What are components?
   - **Answer:**
   ...
   - **If asked more:**
      - Would explain the component hierarchy: `Dashboard` composed `Sidebar`, `Header`, `SensorGrid` with individual `SensorCard` components receiving data via props
3. What is JSX?
   - **Answer:**
   ...
   - **If asked more:**
      - Would discuss JSX expressions using `{}` for JavaScript and how Babel transpiles JSX during build
4. What is state?
   - **Answer:**
   ...
   - **If asked more:**
      - Would discuss component state (useState) vs global state (Context) and the principle of state lifting to the closest common ancestor
5. What are props?
   - **Answer:**
   ...
   - **If asked more:**
      - Would discuss prop drilling, TypeScript interfaces for prop validation, and default props for optional values
6. Difference between state and props.
   - **Answer:**
   ...
   - **If asked more:**
      - Would explain unidirectional data flow: state in parents flows down as props, children communicate up via callback props
7. What are hooks?
   - **Answer:**
   ...
   - **If asked more:**
      - Would mention the rules of hooks: only call at top level, only from React functions
      - We followed these strictly
8. What is `useState`?
   - **Answer:**
   ...
   - **If asked more:**
      - Would discuss initial state, async updates, and the functional form `setCount(prev => prev + 1)` for state depending on previous value
9. What is `useEffect`?
   - **Answer:**
   ...
   - **If asked more:**
      - Would discuss cleanup functions, the dependency array (empty = run once, omitted = run every render), and avoiding infinite loops
10. What is `useMemo`?
    - **Answer:**
    ...
    - **If asked more:**
       - Would distinguish `useMemo` (memoizes a value) from `useCallback` (memoizes a function)
11. What is `useCallback`?
    - **Answer:**
    ...
    - **If asked more:**
       - Would explain that `useCallback(fn, deps)` is essentially `useMemo(() => fn, deps)` and is most useful with optimized child components
12. What is controlled component?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss the difference from uncontrolled components (DOM handles its own state via refs) and why controlled is preferred for validation and dynamic UIs
13. What is uncontrolled component?
    - **Answer:**
    ...
    - **If asked more:**
       - Would explain that uncontrolled components use `useRef` and are less testable since state is not in React
14. What is conditional rendering?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss ternary for if-else, `&&` for simple show/hide, and early returns for complex conditions
15. What is list rendering?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss empty list handling ("no data" message), loading states, and rendering paginated lists efficiently
16. Why is key needed in list rendering?
    - **Answer:**
    ...
    - **If asked more:**
       - Would explain that keys must be stable, unique, and predictable
       - Index as key is acceptable only for static, non-reordered lists
17. What is React Router?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss BrowserRouter vs HashRouter, protected routes with auth checks, and route parameters for sensor details
18. What is RBAC in frontend?
    - **Answer:**
    ...
    - **If asked more:**
       - Would emphasize that frontend RBAC is UX convenience, not security - all sensitive operations must be enforced on the backend
19. How do you protect routes in React?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss handling token expiry by redirecting to login with a session-expired message
20. How do you call APIs from React?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss Axios as an alternative with interceptors for token refresh and request cancellation for unmounted components
21. How do you handle loading state?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss skeleton screens for better UX and handling loading for individual components vs the whole page
22. How do you handle errors in UI?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss error boundaries to prevent the whole dashboard from crashing due to one failed component
23. How do you optimize React performance?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss React DevTools Profiler, virtualization with react-window for large lists, and debouncing rapid state updates
24. What is lazy loading?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss combining lazy loading with Suspense for loading states and Webpack chunking
25. What is code splitting?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss Webpack dynamic imports, analyzing bundle size with source-map-explorer, and splitting vendor libraries for caching
26. How did you build dashboards in React?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss the customizable grid layout using react-grid-layout for drag-and-drop widget positioning and per-widget data fetching to isolate loading/error states
27. What are customizable dashboard grids?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss responsive breakpoints for different screen sizes and how layout preferences persisted across sessions
28. What is Three.js?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss basic Three.js concepts: scene, camera, renderer, and mapping sensor coordinates to 3D positions
29. How did you use Three.js for digital twin visualization?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss loading geometry from JSON, updating colors in real-time using `requestAnimationFrame`, and the performance challenge of rendering hundreds of sensors
30. How do you show live sensor hotspots in UI?
    - **Answer:**
    ...
    - **If asked more:**
       - Would discuss the polling mechanism in `useEffect` with `setInterval`, updating Three.js object material colors, and the interpolation algorithm for the heatmap effect
