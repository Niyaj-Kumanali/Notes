...
1. A stored procedure suddenly becomes slow in production. How do you debug it?
   ...
   - **If asked more:**
      - Examine wait stats
      - Compare actual vs estimated rows in the execution plan
      - Test with OPTION (RECOMPILE) to rule out parameter sniffing
      - Then decide between index fixes or plan guide
2. A query uses an index in testing but not in production. What could be the reason?
   - **Answer:**
   ...
   - **If asked more:**
      - Compare statistics between environments
      - Check parameter sniffing
      - Examine actual execution plans from production
      - Consider index maintenance or query hints after thorough testing
3. A database job takes 5 hours. How would you reduce it?
   - **Answer:**
   ...
   - **If asked more:**
      - Discuss batch processing (10K records at a time)
      - Partition large tables
      - Update statistics
      - Use temp tables for intermediate results instead of CTEs
4. A report has incorrect data after ETL. How do you investigate?
   - **Answer:**
   ...
   - **If asked more:**
      - Compare row counts at each ETL stage
      - Check NULL handling
      - Examine transformation SQL for logic errors
      - Compare against a manually calculated sample
5. Duplicate serial records are entering inventory. How do you prevent it?
   - **Answer:**
   ...
   - **If asked more:**
      - Discuss handling race conditions with Serializable isolation level for check-then-insert
      - Log rejected duplicates for auditing
6. Inventory data is delayed from some channels. How do you handle it?
   - **Answer:**
   ...
   - **If asked more:**
      - Set up monitoring for each channel's submission time
      - Create alerts for missed windows
      - Implement a grace period before marking data as stale
7. Kafka consumer lag is increasing. How do you debug it?
   - **Answer:**
   ...
   - **If asked more:**
      - Check consumer processing rate vs producer rate
      - Look for poison pill messages
      - Consider increasing partitions or consumer instances
      - Optimize processing with batch writes
8. Kafka messages are duplicated. How do you handle it?
   - **Answer:**
   ...
   - **If asked more:**
      - Discuss enabling idempotent producers
      - Track processed message IDs in Redis
      - Design business logic to tolerate duplicates
9. Kafka messages are out of order. How do you handle it?
   - **Answer:**
   ...
   - **If asked more:**
      - Discuss partition keys for per-entity ordering
      - Handle late-arriving data with a tolerance window
      - Consider the tradeoff between ordering and parallelism
10. A Kafka consumer keeps failing on one message. What do you do?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss implementing a dead-letter topic for failed messages
       - Set alerts for DLQ entries
       - Conduct periodic review to fix upstream data issues
11. AWS Lambda is timing out. How do you debug it?
    - **Answer:**
    ...
    - **If asked more:**
       ...
       - Optimize the code (connection reuse, reduce payload)
       - Consider async invocation for long operations
12. EC2 deployment failed after Jenkins pipeline ran successfully. What do you check?
    - **Answer:**
    ...
    - **If asked more:**
       ...
       - Check the compose file for correct image tags
       - Examine application startup logs for connection failures
13. API response time increased suddenly. How do you debug it?
    - **Answer:**
    ...
    - **If asked more:**
       - Check APM tools or CloudWatch metrics
       - Look for slow queries
       - Check for thread pool exhaustion
       - Check Redis cache hit ratios
       - Identify slow endpoints via detailed request logging
14. Database connections are exhausted. How do you debug it?
    - **Answer:**
    ...
    - **If asked more:**
       - Configure HikariCP properly (max pool size, timeout, leak detection)
       - Review code for missing connection closes
       - Add monitoring on connection pool metrics via Actuator
15. Redis cache has stale data. How do you fix it?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss publishing invalidation events on data updates
       - Use shorter TTLs as a quick fix
       - Use write-through caching for data that changes frequently
16. A scheduled job runs on all pods and sends duplicate emails. How do you solve it?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss lock TTL (lease time) to prevent deadlocks
       - Handle lock acquisition failures gracefully
       - Monitor which instance executed each run
17. A scheduled job is missing executions. How do you monitor it?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss exposing job execution metrics via Actuator
       - Create a Grafana dashboard for success/failure rates
       - Configure alerts for missed schedules
18. JWT works locally but fails behind load balancer. What do you check?
    - **Answer:**
    ...
    - **If asked more:**
       - Also check X-Forwarded-Proto and X-Forwarded-For headers
       - Verify Spring Security's requiresSecure() matches the actual protocol
19. Users get 403 after enabling CSRF. How do you fix it?
    - **Answer:**
    ...
    - **If asked more:**
       - Explain that SPAs using JWT in Authorization headers are not vulnerable to CSRF since browsers do not auto-include Auth headers for cross-origin requests
20. Admin role update is not reflected immediately. How do you design it?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss the tradeoff: JWT roles are fast but not immediately updatable; database-checked roles are slower but real-time
       - A hybrid uses short-lived JWTs with DB fallback for sensitive operations
21. Production API returns 500 but logs are unclear. How do you improve observability?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss implementing Actuator for health checks
       - Implement distributed tracing
       - Use centralized log aggregation (CloudWatch or ELK)
       - Set alerting based on error rate thresholds
22. A service dependency is down. How should your service behave?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss circuit breaker to fail fast
       - Use reasonable timeouts (2-3 seconds)
       - Provide fallback responses that let the frontend show appropriate UI like a "data may be delayed" banner
23. A dashboard becomes slow with live data. How do you optimize it?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss WebSocket updates instead of polling
...
24. IoT sensor sends invalid readings. How do you validate them?
    ...
    - **If asked more:**
       - Discuss handling different invalid data types - sensor malfunction (constant values), communication errors (gaps, checksum failures), and out-of-range values - each with different strategy (reject, flag, or interpolate)
25. IoT gateway sends data in bursts. How do you handle backpressure?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss monitoring consumer lag to detect backpressure
       - Scale consumers by increasing partitions
       - Rate limit at ingestion if bursts exceed Kafka capacity
26. A database migration causes backlog. How do you recover safely?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss testing migrations on staging with production data
       - Use backward-compatible migrations
       - Have a rollback script ready before running any migration
27. A deployment needs rollback. What is your process?
    - **Answer:**
    ...
    - **If asked more:**
       - Differentiate code rollback vs database rollback - DB changes must be backward-compatible so code rollback is safe
       - For irreversible changes, deploy a fix forward instead
28. A Docker container works locally but fails on EC2. What do you check?
    - **Answer:**
    ...
    - **If asked more:**
       ...
       - Compare OS architecture
       - Ensure ports are open in the security group
29. A REST API has inconsistent error responses. How do you standardize it?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss defining error codes
       - Use field-level validation errors in standard format
       - Document error schemas in OpenAPI so the frontend handles errors generically
30. A frontend user sees pages they should not access. How do you fix RBAC?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss the principle: frontend RBAC is UX only, backend must always validate permissions
       - The fix adds server-side authorization checks matching frontend route guards
31. A stored procedure returns correct results but is slow. What metrics do you check?
    - **Answer:**
    ...
    - **If asked more:**
       - Use SET STATISTICS TIME and IO for detailed metrics
       - Compare with OPTION (RECOMPILE) to rule out parameter sniffing
...
32. A query is fast for one parameter but slow for another. What could be wrong?
    ...
    - **If asked more:**
       - Use OPTION (RECOMPILE) to test
       - Check parameter data types match column types
       - Look for skewed data distribution
       - Consider OPTION (OPTIMIZE FOR UNKNOWN)
33. A microservice is receiving duplicate requests. How do you make it idempotent?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss idempotency key generation (UUID v4)
       - Store with Redis with TTL or DB table
       - Use response caching
       - Use TTL-based expiry for the key
34. A payment-like operation succeeds but client times out. How do you handle retry?
    - **Answer:**
    ...
    - **If asked more:**
       - Implement a status check endpoint where the client polls for completion using the transaction ID
...
35. A report must be generated exactly once across multiple servers. How do you design it?
    ...
    - **If asked more:**
       - Also use a database table tracking report status (submitted, in-progress, completed) with optimistic locking
       - Schedule cleanup for abandoned "in-progress" entries
36. An API must process 10,000 records. Do you process synchronously or asynchronously?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss the tradeoffs: async adds complexity but prevents timeouts and enables scaling
       - For smaller batches (<100 records), synchronous is simpler
37. A customer asks for near real-time dashboard updates. What architecture would you choose?
    - **Answer:**
    ...
    - **If asked more:**
       - Discuss the pipeline: sensor -> Kafka -> Spring Boot (WebSocket broadcast) -> React
       - For scaling, Redis Pub/Sub coordinates broadcasts across multiple server instances
38. A database table is growing very large. How do you manage performance?
    - **Answer:**
    ...
    - **If asked more:**
       - Use narrow indexes on queried columns
       - Maintain statistics
       - Use page compression for storage/I/O savings
...
39. A third-party API is slow. How do you protect your application?
    ...
    - **If asked more:**
       - Discuss appropriate timeouts (connection + read)
       - Use bulkhead isolation to prevent slow API calls from consuming all threads
...
40. A production issue happens at midnight. How do you approach debugging?
    ...
    - **If asked more:**
       - Review recent deployments before midnight
       - Check monitoring dashboards for the specific time
       - Look for concurrent operations that might conflict
       - Reproduce by running jobs in staging with similar load
