...
5. Duplicate records are entering the system. How do you prevent it?
6. Data from some sources is delayed. How do you handle it?
...
1. A stored procedure suddenly becomes slow in production. How do you debug it?
   - **Answer:**
      ...
      - A common cause is stale statistics after a large data load; updating statistics often restores performance
...
2. A query uses an index in testing but not in production. What could be the reason?
   - **Answer:**
      ...
      - A query may use an index in dev but scan in production because the indexed column has many NULL values in production data
...
3. A database job takes 5 hours. How would you reduce it?
   - **Answer:**
      ...
      - A job can often be reduced from hours to minutes by identifying missing indexes, rewriting correlated subqueries as joins, and breaking the procedure into batch operations
...
4. A report has incorrect data after ETL. How do you investigate?
   - **Answer:**
      ...
      - An incorrect report is often traced to a JOIN causing data duplication; adding DISTINCT and verifying row counts at each stage identifies the issue
...
5. Duplicate records are entering the system. How do you prevent it?
   - **Answer:**
      - Add a unique constraint at the database level and a duplicate check in the application layer
      - Duplicates can be prevented by checking against a verified baseline and rejecting existing records
      - Handle race conditions with Serializable isolation level for check-then-insert patterns to prevent two concurrent requests from both inserting the same record
...
6. Data from some sources is delayed. How do you handle it?
   - **Answer:**
      - Investigate the data flow for each delayed source - check submission timestamps, ETL processing, and error logs
      - Delays are often caused by upstream systems sending data in non-standard formats that failed validation
      - Set up monitoring for each source's submission time to track the average delay and identify trends over time
      - Create alerts for missed windows so that failures to submit data within the expected timeframe are detected promptly
...
7. Kafka consumer lag is increasing. How do you debug it?
   - **Answer:**
      ...
      - Lag often increases when database writes bottleneck during high message traffic
...
8. Kafka messages are duplicated. How do you handle it?
   - **Answer:**
      ...
      - Consumers should be designed idempotently - processing the same message twice should produce the same result
      - Deduplication IDs can be used in the data store to ignore duplicate readings
...
9. Kafka messages are out of order. How do you handle it?
   - **Answer:**
      ...
      - Each event typically includes a timestamp for sorting
      - For strict ordering, a partition key on the entity ID ensures messages for the same entity stay in order
...
10. A Kafka consumer keeps failing on one message. What do you do?
    - **Answer:**
       ...
       - A SeekToCurrentErrorHandler can be configured to retry a few times then send to a DLQ topic
       ...
       - Set alerts for DLQ entries using Kafka monitoring tools so failures are detected immediately
...
11. AWS Lambda is timing out. How do you debug it?
    - **Answer:**
       ...
       - A Lambda can time out due to network connectivity issues with downstream services such as a message broker or database
       - Increase timeout temporarily as a short-term fix while investigating, but always aim to solve the root cause rather than just extending the timeout
...
12. EC2 deployment failed after Jenkins pipeline ran successfully. What do you check?
    - **Answer:**
       ...
       - A deployment can fail because the EC2 instance ran out of disk space from old Docker images not being cleaned up
...
13. API response time increased suddenly. How do you debug it?
    - **Answer:**
       ...
       - Response time can increase when a database query stops using an index after a frontend update changes the query filter, causing full table scans
...
14. Database connections are exhausted. How do you debug it?
    - **Answer:**
       ...
       - A missing connection pool configuration for a long-running report query can consume all connections
...
15. Redis cache has stale data. How do you fix it?
    - **Answer:**
       ...
       - Set appropriate TTLs and invalidate cache entries when new data arrives
       ...
       - Use shorter TTLs as a quick fix to reduce the staleness window while implementing proper event-driven invalidation
...
16. A scheduled job runs on all pods and sends duplicate emails. How do you solve it?
    - **Answer:**
       ...
       - `@SchedulerLock` can be used where the first Spring Boot instance to acquire the lock runs the job
...
17. A scheduled job is missing executions. How do you monitor it?
    - **Answer:**
       ...
       - A health check endpoint can report the last job run time and alert if overdue
...
18. JWT works locally but fails behind load balancer. What do you check?
    - **Answer:**
       ...
       - JWT can fail behind a load balancer because SSL termination changes the protocol, and the `secure` flag on JWT cookies may not match
...
19. Users get 403 after enabling CSRF. How do you fix it?
    - **Answer:**
       - For REST APIs used by a SPA, CSRF protection can be disabled because it is not needed for token-based authentication - CSRF is primarily for cookie-based auth
       - Call `http.csrf().disable()` in Spring Security
...
20. Admin role update is not reflected immediately. How do you design it?
    - **Answer:**
       ...
       - Roles stored in JWT require re-login
...
21. Production API returns 500 but logs are unclear. How do you improve observability?
    - **Answer:**
       ...
       - Unclear logs can be fixed by adding method-level logging with input parameters and context
       - Implement Spring Boot Actuator for health checks to give an at-a-glance view of all dependency health status without digging through logs
       - Implement distributed tracing with tools like Micrometer + Zipkin or AWS X-Ray to follow a single request across multiple services and identify exactly where it failed
       ...
       - Set alerting based on error rate thresholds to be notified proactively when the 500 error rate spikes rather than waiting for user reports
22. A service dependency is down. How should your service behave?
    - **Answer:**
       ...
       - When a dependency is unavailable, the API can return stale data from Redis with a `stale: true` flag
...
23. A dashboard becomes slow with live data. How do you optimize it?
    - **Answer:**
       ...
       - Update data at reasonable intervals (e.g. every 30 seconds) instead of on every reading and pre-aggregate time-series data in the database
...
24. IoT sensor sends invalid readings. How do you validate them?
    - **Answer:**
       ...
       - A Lambda can validate incoming MQTT messages before publishing to Kafka
...
25. IoT gateway sends data in bursts. How do you handle backpressure?
    - **Answer:**
       ...
       - When gateways reconnect and send bursts, Kafka queues messages and consumers catch up gradually without data loss
...
26. A database migration causes backlog. How do you recover safely?
    - **Answer:**
       ...
       - If a migration caused failures, restore from backup with point-in-time recovery, fix the script, and re-run during low-traffic hours
       ...
       - Have a rollback script ready before running any migration, and verify it works in staging so a revert can be done quickly if the migration causes unexpected issues
27. A deployment needs rollback. What is your process?
    - **Answer:**
       - Stop the current deployment, restore the previous Docker image tag from Jenkins artifacts, and redeploy
       - Keeping the last three successful image tags makes rollback straightforward: pull the previous tag, restart containers, and run smoke tests
...
28. A Docker container works locally but fails on EC2. What do you check?
    - **Answer:**
       ...
       - A Spring Boot container can fail on EC2 because the instance has less memory, causing the JVM to hit its limit and get OOM-killed by Docker
       ...
       - Check file permissions for mounted volumes, since Linux EC2 instances may run the container as a different user than the local machine
...
29. A REST API has inconsistent error responses. How do you standardize it?
    - **Answer:**
       ...
       - A custom ErrorResponse DTO ensures every error has the same structure
...
30. A frontend user sees pages they should not access. How do you fix RBAC?
    - **Answer:**
       ...
       - Route guards should check user roles from JWT, and the backend should always enforce `@PreAuthorize` on APIs
...
32. A query is fast for one parameter but slow for another. What could be wrong?
    - **Answer:**
       ...
       - A date-filtered procedure may be fast for a small date range but slow for a large range
...
33. A microservice is receiving duplicate requests. How do you make it idempotent?
    - **Answer:**
       ...
       - A batch reference ID or similar unique key can serve as the idempotency key
       - Generate idempotency keys using UUID v4 for globally unique identifiers that clients can include in every request
       - Store idempotency keys in Redis with TTL for fast lookups, or in a database table if cached responses need to be returned for duplicate requests that arrive hours later
...
37. A customer asks for near real-time dashboard updates. What architecture would you choose?
    - **Answer:**
       ...
       - For sub-second real-time, WebSocket is better - the backend pushes updates to clients as they arrive from the message broker
       - The pipeline flows: data source -> message broker -> application server (WebSocket broadcast) -> client
...
38. A database table is growing very large. How do you manage performance?
    - **Answer:**
       ...
       - Time-series databases handle this naturally with time-based shards; for relational databases, partition by month with a data retention policy
...
39. A third-party API is slow. How do you protect your application?
    - **Answer:**
       ...
       - If a third-party API dependency is slow, the system can return last known data with a "stale" indicator
...
40. A production issue happens at midnight. How do you approach debugging?
    - **Answer:**
       ...
       - Midnight is commonly when ETL pipelines run, so a midnight issue is likely ETL-related
