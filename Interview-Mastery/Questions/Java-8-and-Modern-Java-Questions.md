...
1. What are Java 8 features?
   - **Answer:**
      ...
      - Lambdas and streams are commonly used daily for collection processing — filtering data, mapping DTOs, and collecting reports without loops
      ...
      - The most commonly used features are Stream API for declarative collection processing and Optional for null-safety in repository queries
2. What is lambda expression?
   - **Answer:**
      - Lambda expressions allow passing behavior as a method argument concisely
      - Lambdas are commonly used in Stream API to filter data above a threshold and map it to domain objects without writing verbose anonymous classes
      ...
      - A common pitfall is trying to modify a captured variable inside the lambda, which causes a compile error — if state mutation is needed, use an `AtomicReference` or a collection
3. What is functional interface?
   - **Answer:**
      - A functional interface has exactly one abstract method and can be implemented by a lambda
      - Built-in ones like Predicate, Function, Consumer, and Supplier are used heavily in Stream API operations for data processing pipelines
      - To create a custom functional interface, define an interface with a single abstract method — e.g., `interface AlertMapper<T, R> { R map(T item); }`
      ...
      - Example: `Comparator` has one abstract method `compare(T o1, T o2)` and many default methods like `thenComparing()`, so it is a functional interface — it can be used as `Comparator.comparing(Employee::getSalary)` with a lambda
4. What is `@FunctionalInterface`?
   - **Answer:**
      ...
      - It is commonly used when defining custom functional interfaces for specific transformation logic in validation frameworks
      ...
      - If a second abstract method is accidentally added, the compiler produces: "Multiple non-overriding abstract methods found in interface"
...
5. What is Stream API?
   - **Answer:**
      ...
      - Streams are commonly used to filter, sort, and collect records by category, replacing complex for-loops with readable one-liners
      ...
      - `Collectors.toMap()`, `Collectors.groupingBy()`, and `Collectors.partitioningBy()` are essential for aggregation — `groupingBy` is commonly used to bucket records by category for reports
6. Difference between collection and stream.
   - **Answer:**
      - Collections store all elements in memory, while streams compute elements on demand and cannot be reused
      - Collections are used as data storage and streams for transformation pipelines — the data stays in the collection, streams process it once
...
7. Difference between intermediate and terminal operations.
   - **Answer:**
      ...
      - In the common filter-and-collect pattern, filter and map just build a pipeline until collect executes everything
...
8. What is lazy evaluation in streams?
   - **Answer:**
      ...
      - Chaining multiple filters on a large collection does not process elements until collect() is called, enabling optimization like operation fusion
      ...
      - Lazy pipelines can be debugged using `peek()` to inspect elements as they flow through the pipeline — `.filter(x -> x > 5).peek(System.out::println).map(...)`
...
9. Difference between `map()` and `flatMap()`.
   - **Answer:**
      ...
      - map is used for simple DTO conversion (Entity->ResponseDTO) and flatMap when each source element produces multiple output objects
      ...
      - With `Collectors.groupingBy`, flatMap-like behavior can be achieved by grouping and then aggregating nested lists
...
10. Difference between `filter()` and `map()`.
    - **Answer:**
       ...
       - Filtering before mapping reduces processing — e.g., filter valid records then map to response DTOs in API layers
       ...
       - Combining `filter` + `map` is idiomatic in Stream pipelines — filter first to reduce the dataset, then map to extract or transform the fields needed
...
11. Difference between `findFirst()` and `findAny()`.
    - **Answer:**
       ...
       - Use cases: `findAny()` when any valid result suffices (e.g., "find any available supplier") and `findFirst()` when order matters (e.g., "find the earliest event by timestamp")
...
12. Difference between sequential and parallel streams.
    - **Answer:**
       ...
       - Parallel streams are generally avoided for small collections or I/O-bound operations where parallelism adds overhead
...
13. When should you avoid parallel streams?
    - **Answer:**
       - Avoid parallel streams for small datasets, I/O-bound operations (DB calls, HTTP), operations with shared mutable state, and ordered streams where merge overhead outweighs gain
       - Collections of a few thousand records are generally too small to benefit from parallelism
...
14. What is `Optional`?
    - **Answer:**
       ...
       - Optional is commonly used when fetching data by ID from a repository, then handling present/empty cases with orElseThrow
...
15. Why should we not use `Optional.get()` directly?
    - **Answer:**
       ...
       - Instead, orElse(), orElseThrow(), or orElseGet() should be used to provide safe default values or custom exceptions
       - `orElseThrow()` is preferred for "must exist" cases — it throws a meaningful exception (e.g., `new EntityNotFoundException("Entity not found: " + id)`) instead of a generic `NoSuchElementException`
...
16. Difference between `orElse()` and `orElseGet()`.
    - **Answer:**
       ...
       - In performance-sensitive code, orElseGet should be used to avoid unnecessary object creation
       ...
       - A common bug: calling `orElse(new ExpensiveObject())` on every query result creates an unnecessary object each time, wasting memory and CPU; switching to `orElseGet(ExpensiveObject::new)` fixes it
...
17. What are method references?
    - **Answer:**
       ...
       - They are commonly used in stream pipelines — e.g., `.map(DTO::new)` for constructor references or `.forEach(logger::info)` for method calls
...
18. What are default methods?
    - **Answer:**
       ...
       - They are useful when extending functional interfaces with convenience methods, like adding a default `orElseThrow` method to a custom validation interface
       ...
       - If two sibling interfaces have the same default method and the class does not override it, a compilation error occurs — the diamond problem. It is resolved by explicitly overriding the method in the implementing class
19. What is `CompletableFuture`?
    - **Answer:**
       - CompletableFuture is a Future that can be manually completed and supports chaining async operations
       - CompletableFuture is commonly used to fetch data from multiple sources asynchronously and combine results without blocking
       ...
       - `allOf(future1, future2, future3)` combines multiple futures — it is commonly used to fetch data from multiple services in parallel and join the results only when all complete
...
20. Difference between `thenApply()` and `thenCompose()`.
    - **Answer:**
       ...
       - thenApply is used for simple transformations and thenCompose to chain dependent async calls
...
21. Difference between `thenApply()` and `thenAccept()`.
    - **Answer:**
       ...
       - thenAccept is used when a side effect (like logging or caching) needs to be performed after a future completes without returning a new value
...
22. How do you handle exceptions in `CompletableFuture`?
    - **Answer:**
       - Use exceptionally() to recover from errors with a fallback value, handle() to process both success and failure, and whenComplete for side-effect cleanup
       - In async pipelines, errors can be logged via exceptionally and fallback values returned
...
23. What is Java Date and Time API?
    - **Answer:**
       ...
       - LocalDate is commonly used for date-only fields and Instant for UTC timestamps, replacing the flawed java.util.Date
...
24. Difference between `LocalDateTime`, `ZonedDateTime`, and `Instant`.
    - **Answer:**
       ...
       - Instant is commonly used for storing timestamps in time-series databases, ZonedDateTime for user-facing displays, and LocalDate for date-only fields
...
25. What are records in Java?
    - **Answer:**
       ...
       - Records are commonly used in DTO layers for API response objects to reduce boilerplate and ensure immutability
...
26. What are sealed classes?
    - **Answer:**
       ...
       - Sealed classes are useful for domain event types — like `SealedEvent permits SensorEvent, AlertEvent, SystemEvent` — for exhaustive pattern matching
       ...
       - Exhaustive switch (Java 17+) with sealed types guarantees all cases are handled — the compiler enforces that every permitted type has a case, so the `default` clause can be omitted safely
...
27. What is pattern matching?
    - **Answer:**
       ...
       - In Java 16+, pattern matching for instanceof is available: `if (obj instanceof String s)` which eliminates separate casting, making code cleaner
...
28. What are switch expressions?
    - **Answer:**
       ...
       - Switch expressions are commonly used when mapping enum values to strings or numeric thresholds, without break statements
...
29. What are text blocks?
    - **Answer:**
       ...
       - They are commonly used for SQL queries — embedding multi-line queries without concatenation or escape sequences for newlines
...
30. What are virtual threads?
    - **Answer:**
       ...
       - Virtual threads are particularly useful for handling high-volume message processing where each message can be handled by its own virtual thread
...
31. Given a `List<Employee>` where each employee has Name, ID, Salary, and Experience, filter employees based on experience and salary criteria, then print the results in ascending or descending order. Write the Java code.
   - **Answer:**
      - Using the Stream API, filter with a `Predicate` and sort with `Comparator`
   ...
   - To tie-break by experience, chain comparators with `Comparator.comparing(Employee::getSalary).thenComparing(Employee::getExperience)`
