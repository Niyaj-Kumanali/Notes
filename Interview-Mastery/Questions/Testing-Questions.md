...
1. What is unit testing?
   - **Answer:**
      ...
      - Typically involves writing JUnit + Mockito tests for a validation service — mocking the repository and testing validation logic with various input formats
...
2. What is integration testing?
   - **Answer:**
      ...
      - A typical approach is using `@SpringBootTest` with Testcontainers to test a full Kafka consumer to database flow
      - Integration tests catch configuration issues that unit tests miss — wrong connection strings, mismatched serialization formats, missing bean wiring, incorrect Spring profiles. A balanced coverage strategy writes unit tests for all business logic and integration tests for critical paths: database queries, message publishing/consuming, and API endpoint wiring
3. Difference between unit and integration test.
   - **Answer:**
...
5. What is Mockito?
   - **Answer:**
      ...
      - In service-layer tests, `when(repository.findById(any())).thenReturn(Optional.of(entity))` allows testing business logic without needing a database
...
6. What is mock?
   - **Answer:**
      ...
      - When testing a service, mocking the repository to return predefined data makes tests fast, isolated, and deterministic
      - Mocks vs stubs: stubs are state-based — predefined return values are set up and the test passes if the code produces the right result. Mocks are interaction-based — expectations are set (this method must be called exactly once with these arguments) and those interactions are verified after execution. Mockito mocks combine both capabilities: behavior can be stubbed and calls verified on the same mock
7. What is stub?
   - **Answer:**
      - Returns predefined responses to method calls, making the test environment predictable
      - Stubs can be used for stored procedure calls to return known result sets, testing reporting logic without running the actual procedure
      - Stubs focus on state testing — provide data, run the code, assert the output. Mocks focus on interaction behavior — verify that the code called the right methods in the right order with the right arguments. A stub does not care if it is called once or ten times; a mock will fail if expectations are not met
8. Difference between mock and spy.
   - **Answer:**
      ...
      - Mocks are typically used for repositories (completely fake) and spies when some real behavior is needed but specific methods should be overridden
      - Prefer mocks by default — they are explicit about behavior and have no hidden side effects. Spies call real methods by default, which means they can trigger side effects (database writes, network calls) if the real implementation is not carefully managed. Use a spy only when there is a genuine need to test a real class with one or two methods overridden
9. What is `@Mock`?
   - **Answer:**
      - A Mockito annotation that creates and injects a mock for the annotated field
      - Repository dependencies can be annotated with `@Mock` in test classes, using `MockitoAnnotations.openMocks(this)` or `@ExtendWith(MockitoExtension.class)` for initialization
...
10. What is `@InjectMocks`?
   - **Answer:**
      ...
      - In service tests, the service class is annotated with `@InjectMocks`, and Mockito automatically injects the `@Mock` repositories, eliminating manual constructor calls
...
11. What is `@MockBean`?
   - **Answer:**
      ...
      - In `@WebMvcTest` controller tests, `@MockBean` is used to mock service layer beans while testing only the controller layer
...
12. What is `@SpringBootTest`?
   - **Answer:**
      ...
      - It can be used with Testcontainers to test a full flow from controller to message broker to database, verifying Spring bean wiring and configuration
...
13. What is `@WebMvcTest`?
   - **Answer:**
      ...
      - Used with `@MockBean` for services, it allows testing endpoint mappings, request validation, status codes, and error responses without loading the full context
...
14. What is Testcontainers?
   - **Answer:**
      ...
      - Testcontainers can be used to spin up real Kafka and database containers during tests, ensuring tests run against the same versions as production
...
15. Why use Testcontainers?
   - **Answer:**
      ...
      - When an in-memory database does not support all features of the production database (e.g., stored procedures, specific SQL dialects), Testcontainers with a real database container catches compatibility issues earlier
      - The tradeoff: container startup adds seconds to test execution, but the gain is confidence that tests reflect real behavior. Testcontainers tests can run in CI on every pull request while being excluded from local fast-feedback loops during development. For cases where container startup overhead is too high, a shared container can be reused via `TestcontainersReuse`. The reliability gain outweighs the speed cost for critical integration paths
16. How do you test repository layer?
   - **Answer:**
      - Use `@DataJpaTest` which loads only JPA beans and uses an embedded or Testcontainers database
      - Custom queries, pagination, and sorting can be tested — verifying derived queries generate correct SQL and return expected results
      - `@DataJpaTest` auto-configures an in-memory database, `EntityManager`, and repository beans. For native queries and `@Query` annotations, test with parameter binding using `@Param` and verify results against expected datasets loaded with `@Sql` or `TestEntityManager`. Projection interfaces are tested by asserting the returned DTO contains only the selected fields with correct values
17. How do you test service layer?
   - **Answer:**
      - Use JUnit and Mockito, mocking repository dependencies and verifying business logic
      - The repository is mocked, then validation rules, error handling, and edge cases like duplicate entries and invalid formats are tested
...
18. How do you test controller layer?
   - **Answer:**
      - Use `@WebMvcTest` with MockMvc, mocking service layer beans
      - GET endpoints can be verified to return correct JSON, POST with invalid data to return 400 with validation errors, and endpoints to require proper authentication
...
19. How do you test Kafka consumer?
   - **Answer:**
      - Use `@EmbeddedKafka` from Spring Kafka test for lightweight tests or Testcontainers with a real Kafka container
      - Test messages can be published, the consumer can be verified to process them, and expected data can be asserted in the target data store
...
20. How do you test scheduled jobs?
   - **Answer:**
      - Extract the job logic into a testable service and test that directly, plus an integration test that triggers the scheduler and verifies the outcome
      - A reconciliation job, for example, can be tested by calling the reconciliation service directly with test data
      - Do not test the scheduler framework itself — `@Scheduled` is Spring's responsibility. Focus on the logic: ensure idempotency by running the job twice with the same data and verifying no duplicate processing. Test failure handling by injecting exceptions and verifying the job logs errors and continues rather than crashing. Use `@Scheduled(fixedDelay = ...)` rather than `fixedRate` so jobs do not overlap. For integration tests, manually trigger the scheduled method and assert side effects
21. How do you test security rules?
   - **Answer:**
      - Use `@WithMockUser` for role-based access and manually set authentication for token-based tests
      - Unauthenticated requests should return 401, wrong roles should get 403, and correct roles should access endpoints
...
22. What is code coverage?
   - **Answer:**
      ...
      - JaCoCo is commonly used with Maven, targeting 70-80% coverage for service layers with clear exclusion rules for DTOs, configurations, and generated code
      - High coverage number does not guarantee good tests — a test that calls a method without assertions still counts toward coverage. Branch coverage is more meaningful than line coverage: it verifies both true and false paths of `if` statements are exercised. Branch coverage should be focused on business-critical code (validation, calculations, error handling) and boilerplate excluded: Lombok-generated getters/setters, configuration classes, DTOs, and repository interfaces with only derived queries
23. Is 100% coverage always good?
   - **Answer:**
      - No. 100% coverage can be misleading if tests only check simple paths with no meaningful assertions
      - Coverage should be focused on business-critical code — validation logic, calculations, error handling — not on boilerplate getters or configuration classes
...
24. What is regression testing?
   - **Answer:**
      ...
      - In a CI pipeline, the full test suite can run on every pull request — unit, integration, and smoke tests — catching regressions before they reach production
      - Automated regression testing reduces the fear of making changes, especially for critical business paths where a regression could cause financial discrepancies. A comprehensive regression suite acts as a safety net: developers refactor and add features confidently because the suite catches unintended side effects. Regression tests can be categorized by priority — critical business flows run first, full suite runs in parallel. Failed regressions block the merge
25. What is smoke testing?
   - **Answer:**
      - A quick check that the application starts and core functionality works
      - After deploying to staging, smoke tests can check the health endpoint returns 200, a simple API call works, and the dashboard loads — all within 60 seconds
...
26. What is API testing?
   - **Answer:**
      ...
      - Postman can be used for manual testing and automated API tests can be included in the CI pipeline using Spring Boot's TestRestTemplate
...
27. What is Postman?
   - **Answer:**
      ...
      - Postman collections can be created for each API module with environment variables, pre-request scripts for authentication, and test scripts for assertions
      - Collections can be version-controlled in the repo and used for QA testing and CI via Newman (Postman's CLI runner). Newman runs collections as part of the CI pipeline, failing the build if any request returns an unexpected status code or response. Environment variables allow the same collection to run against local, staging, and production. Pre-request scripts handle token refresh so tests always have valid authentication
28. How do you test error cases?
   - **Answer:**
      - Intentionally send invalid inputs and assert the correct error response
      - Missing fields, invalid formats, and duplicate entries can be tested — verifying 400/422 status codes and checking that error messages are actionable
...
29. How do you test performance changes?
   - **Answer:**
      - Run the same workload before and after changes, measuring response times, throughput, and resource usage
      - For database performance optimization, stored procedures can be run with realistic data, execution plans recorded, and elapsed time measured — potentially reducing execution time from hours to minutes
      - Consistent test conditions are essential: same data volume, same server load, warm caches (run the query once to populate the buffer pool before measuring). Tools like JMeter simulate realistic concurrent load — not just single-user response time, but throughput under 50, 100, or 500 concurrent users. Compare p50, p95, and p99 latencies, not just averages. Monitor CPU, memory, and I/O during the test to ensure the optimization did not shift the bottleneck elsewhere
30. How do you validate database optimization results?
   - **Answer:**
      - Compare execution plans before and after changes, checking index usage (seek vs scan), logical reads, and actual execution time with realistic data
      - Execution plans can be captured, expensive operators identified, changes applied, and index seeks verified to have replaced scans
