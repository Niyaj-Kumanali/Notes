...
1. What are design principles and design patterns?
   - **Answer:**
      ...
      - Principles tell developers *how to think* about design; patterns give developers *ready-made templates* to solve specific problems
...
2. Explain the SOLID principles.
   - **Answer:**
      ...
      - In a well-structured service layer, each service is kept focused on one domain (single responsibility) and interfaces like `PartnerRepository` are injected rather than concrete classes (dependency inversion)
...
3. What are the DRY and KISS principles?
   - **Answer:**
      ...
      - DRY is applied by putting validation rules in a single utility method, and KISS by choosing a straightforward loop or stream over a complex pattern when the simple version is enough
      ...
      - For example: extracting a serial-number validation helper into a utility method and reusing it across validation and reporting code keeps both DRY and simple
4. Explain the Factory design pattern.
   - **Answer:**
      ...
      - Instead of calling `new ConcreteClass()` in client code, a factory is asked to create it — this decouples client code from concrete implementations
      - For example, a factory could decide which report generator (`PDFReportGenerator`, `ExcelReportGenerator`) to create based on the requested report format
...
5. Explain the Singleton design pattern.
   - **Answer:**
      ...
      - It would be used for a shared resource like a database connection manager or a configuration reader
      - Thread-safety matters: a naive lazy singleton is broken under concurrency, so either eager init, double-checked locking (with `volatile`), or the Bill Pugh static holder class (which is the cleanest approach and avoids synchronization overhead) should be used
      - Singletons are often criticized because they act like global state and hurt testability — in Spring, the singleton scope gives the same single-instance guarantee but through the IoC container, so hand-rolling a Singleton in Spring apps is rarely necessary
6. Explain the Strategy design pattern.
   - **Answer:**
      - Strategy is a behavioral pattern that defines a family of algorithms, puts each in its own class, and makes them interchangeable at runtime
      - The client holds an interface reference and the algorithm can be swapped without changing the client
      - For example, alert strategies (e.g., `ThresholdAlertStrategy`, `TrendAlertStrategy`) can be defined and switched per sensor type
...
7. Explain the Observer design pattern.
   - **Answer:**
      ...
      - For example, when a sensor reading exceeds a threshold, the subject can notify observers like an alert service and a dashboard updater without the sensor code knowing about them
...
8. Explain the Adapter design pattern.
   - **Answer:**
      ...
      - For example, if a third-party reporting library exposes a different API than expected, an adapter class can be created implementing the expected interface and delegating to the third-party class inside
      ...
      - Another example: using an adapter to bridge a legacy LDAP client API to an `AuthenticationService` interface, so the rest of the codebase works against a consistent contract
