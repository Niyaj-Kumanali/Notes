...
1. Explain your current role at Talentpace Pvt Ltd.
   - **Answer:**
      ...
      - Inventory validation engine: designed validation rules in MSSQL using set-based operations over cursors, which directly drove the 15-second processing time
2. What are your main responsibilities in your current project?
   - **Answer:**
...
4. Explain the Lenovo Channel Data Management System.
   - **Answer:**
      ...
      - Four ETL stages: staging, validation, transformation, and reporting; worst bottleneck was the transformation stage with a cursor-based loop processing partners one-by-one instead of as a set
5. What problem did CDMS solve for Lenovo India?
   - **Answer:**
      ...
      - The reporting team started at 8 AM, so a 5-hour procedure meant stale data in executive dashboards and delayed partner incentive decisions
6. What were the main modules in CDMS?
   - **Answer:**
...
9. What kind of execution plan issues did you identify?
   - **Answer:**
      ...
      - Diagnosed implicit conversions via yellow warning triangles in the execution plan, then checked column vs. parameter data types in the `WHERE` clause
10. What kind of indexes did you create or modify?
    - **Answer:**
...
13. What was the most difficult part of optimizing CDMS?
    - **Answer:**
       ...
       - A GOTO-based error handler masked a real bug — it caught an error, logged it, then jumped back into the middle of the loop, causing duplicate processing of certain partners
14. What is partner tagging in CDMS?
    - **Answer:**
...
16. What were the 4 ETL pipeline stages in CDMS?
    - **Answer:**
       ...
       - Stage 3 (Transform) was the bottleneck; incentive calculation took 5 hours due to the cursor-based partner loop and missing indexes
17. How did you reduce reporting discrepancies by 90%?
    - **Answer:**
...
30. Explain the Cold-Chain Monitoring System.
    - **Answer:**
       ...
       - Example flow: sensor reads 8°C (above 6°C threshold) → IoT Core → Lambda → Kafka → Spring Boot → InfluxDB → Grafana alert triggers → operations team notified within 30 seconds
31. What was your role in the Cold-Chain Monitoring System?
    - **Answer:**
...
35. Why did you use AWS Lambda?
    - **Answer:**
       ...
       - On Lambda parse failure or Kafka producer exception, the raw payload was written to an S3 bucket with a timestamp and error code for later replay
36. Why did you use Kafka in the cold-chain project?
    - **Answer:**
       ...
       - Single topic `cold-chain-sensor-data` with 6 partitions, messages partitioned by `room-id` for ordering; 3 Spring Boot consumers in the same group for parallel processing
37. Why was Kafka running on EC2?
    - **Answer:**
...
40. How did you handle sensor data bursts?
    - **Answer:**
       ...
       - Health check endpoint in Spring Boot dynamically reduced the consumer's `max.poll.records` when Kafka consumer lag grew beyond 10,000 messages, preventing memory exhaustion
41. How did you handle delayed IoT messages?
    - **Answer:**
...
42. How did you handle duplicate IoT messages?
    - **Answer:**
       ...
       - MSSQL primary-key check would add 5-10ms latency per message; Redis in-memory SET operations were orders of magnitude faster at 800 msg/s
43. How did you handle invalid sensor readings?
    - **Answer:**
       ...
       - Example: a sensor once started reporting -40°C due to a hardware fault; the validation rule caught it, the invalid reading was logged, and the maintenance team replaced the sensor before it caused a false temperature excursion alert
44. Why did you use InfluxDB?
    - **Answer:**
...
48. How did you reduce temperature excursions by 85%?
    - **Answer:**
       ...
       - Metric: (excursions before system - excursions after system) / excursions before system × 100, comparing 6 months before deployment to 6 months after on the same set of cold rooms
49. How did the system reduce energy costs by 18%?
    - **Answer:**
       ...
       - Average room: ₹15,000/month before → ₹12,300/month after; ₹2,700/month savings per room × 200 rooms
50. How did you improve load time by 80%?
    - **Answer:**
...
51. How did you improve responsiveness by 30%?
    - **Answer:**
       ...
       - Used Spring Boot's `WebSocketHandler` with `SimpMessagingTemplate` broadcasting the latest reading to the `/topic/latest-readings` channel whenever a new data point was persisted
52. What was the role of Redis in the cold-chain project?
    - **Answer:**
       ...
       - Dedup cache: ~200K keys × ~100 bytes = ~20 MB; latest-reading cache: 600 keys × ~200 bytes = ~120 KB; both fit in a single t3.micro's 500 MB allocation
53. How did you secure APIs with Spring Security?
    - **Answer:**
...
58. How did Docker help in deployment?
    - **Answer:**
       ...
       - Custom bridge network `cold-chain-net` so containers could communicate by service name (e.g., `api:8080`, `redis:6379`) instead of environment-specific IP addresses
59. What was the role of AWS S3?
    - **Answer:**
...
61. What was the role of MQTT?
    - **Answer:**
       ...
       - 600 unique topics across 20 locations, 10 rooms per location, 3 sensors per room; IoT Core wildcard subscription `cold-chain/+/+/+` captured all messages
62. What was the role of Databricks?
    - **Answer:**
...
66. Which project had the most technical complexity?
    - **Answer:**
       ...
       - One complex failure: Kafka broker disk filled up because retention wasn't configured correctly after a schema change increased message size by 3x; consumers couldn't commit offsets, which caused rebalancing and duplicate downstream writes
67. What would you improve if you redesigned one of your projects?
    - **Answer:**
...
70. How do you explain your project architecture to a senior engineer?
    - **Answer:**
       ...
       - Critical design decisions: Kafka for decoupling (handles bursts, provides replay), Redis for sub-millisecond dedup and caching (avoids InfluxDB load), and InfluxDB's automatic downsampling for cost-effective long-term storage
