...
> **Interview follow-up:** The `DatabaseConfig` object is now immutable with a builder, but some configuration values depend on each other — if `ssl=true`, then `sslCertPath` is required. How do you enforce cross-field validation in the Builder without making it mutable after construction?
...
> **Interview follow-up:** Your adapter calls the legacy `XMLOrderService` which takes 3 seconds per request — the adapter hides this latency, so the callers don't know they're making a slow remote call instead of an in-process operation. Is the adapter masking a performance problem, and how do you surface the true cost?
...
> **Interview follow-up:** A new requirement adds hybrid formats (e.g., JSON with embedded Excel tables) — the pure Strategy pattern assumes one format per export. How do you compose strategies without adding format-specific conditionals back into the exporter?
...
- **Leverage Spring's built-in pattern implementations** — Spring already implements Proxy (`@Transactional`, `@Cacheable`), Template Method (`JdbcTemplate`, `JpaRepository`), Factory (`@Bean`, `BeanFactory`), and Observer (`@EventListener`). Don't reimplement these patterns. Instead, understand which pattern Spring uses and extend it. For custom proxies, use Spring AOP rather than generating CGLIB proxies manually. A team hand-rolled custom AOP proxies using reflection for a `@AuditLog` annotation before discovering Spring AOP — the custom implementation didn't handle proxy self-invocation correctly, causing silent audit gaps on all internal method calls for 6 months.

- **Don't force patterns where simpler solutions work** — A simple `if-else` for 2-3 cases is more readable than a full Strategy pattern with interfaces, implementations, and a factory. Add patterns as complexity grows, not preemptively. YAGNI applies to design patterns too — premature abstraction adds indirection without benefit. A startup applied the Command pattern to all three of their REST endpoints because "it scales better" — the resulting code was 4x larger with no actual benefit, and a junior developer spent 2 days tracing through `CommandDispatcher` → `HandlerRegistry` → `AbstractCommandHandler` to understand a simple user creation flow.
