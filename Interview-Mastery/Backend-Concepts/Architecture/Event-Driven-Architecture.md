...
- **Make every consumer idempotent** — At-least-once delivery guarantees that duplicates will happen. Every event consumer must handle the same event twice without side effects. Use dedup stores (Redis with TTL), upsert operations instead of inserts, and idempotency keys in external API calls. Test idempotency by replaying events in staging.
