...
1. **Benefits**:
   ...
   - **Pre-compilation** — Procedure is parsed and optimized on first execution; subsequent calls reuse the cached plan.
   ...
   - **Transaction control** — Complex multi-step operations wrapped in a single transaction.
   - **Deterministic logic** — Consistent behavior regardless of client implementation.

2. **Procedure vs Function**:

   - **Function** — Must return a value. Can be used in SQL expressions (`SELECT func()`). Limited transaction control.
   - **Procedure** — May not return a value. Called via `CALL proc()`. Full transaction control including COMMIT/ROLLBACK.

3. **Languages**:

   - **PL/pgSQL** — PostgreSQL's native procedural language.
   - **PL/SQL** — Oracle's proprietary language.
   - **T-SQL** — SQL Server's language.
   - **SQL/PSM** — MySQL/DB2 standard.

4. **Security Contexts**:

   - **SECURITY INVOKER** (default) — Runs with the caller's permissions.
   - **SECURITY DEFINER** — Runs with the procedure owner's permissions. Used for controlled privilege escalation.
...
### 2. Calling Stored Procedures with JPA
...
### 4. Batch Processing with Cursor
...
### 5. Performance: Set-Based vs Row-by-Row
...
### 6. Security with SECURITY DEFINER
...
2. **Overusing stored procedures for simple CRUD** — application-level JPA is simpler and more testable.
3. **Ignoring error handling** — unhandled exceptions can leave partial transactions.
4. **Hard to unit test** — stored procedures require database setup; prefer integration tests.
5. **Version control challenges** — ensure procedures are in Flyway/Liquibase migrations.
6. **Debugging difficulty** — use `RAISE NOTICE` / `DBMS_OUTPUT`, not breakpoints.
...
### 1. Funds Transfer with Full Transaction Control
...
### 2. Batch Order Processing with Cursor
...
### 3. Row-Level Access Control via SECURITY DEFINER
...
1. **Q: You are designing a payment processing system. The team proposes putting all business logic in stored procedures for "performance." The system needs to call external payment APIs, send emails, and integrate with fraud detection. What do you advise?**
...
2. **Q: A stored procedure processes 50,000 rows using a FOR loop with individual UPDATEs. Each iteration takes 2ms, total 100 seconds. How do you make it 100x faster?**
...
3. **Q: A stored procedure was deployed via Flyway but continues to exhibit old behavior. The new definition is in pg_proc. Why is the old behavior cached?**
   A: PostgreSQL caches execution plans. The new definition exists but the cached plan is stale. Run `DISCARD PLANS` or wait for cache invalidation. For functions affected by schema changes, use `ALTER FUNCTION ... WITH (security_invoker=false)`.

4. **Q: Your team needs to grant a reporting user access to see order data but prevent them from seeing `credit_card_last_four`. You don't want a view. How do you use stored procedures for this?**
...
5. **Q: A nightly ETL stored procedure takes 3 hours. If it fails at 2.5 hours, all changes roll back. How do you make it resumable?**
...
6. **Q: A stored procedure with SECURITY INVOKER fails with permission errors when called by the application user, even though the user has EXECUTE privilege. What's wrong?**
...
7. **Q: A microservice needs data from another service's database. The team wants a cross-service stored procedure. Is this a good idea?**
   A: No — this creates tight coupling. Each service owns its database. Call the other service's API instead. For cross-database queries, use foreign data wrappers (FDW) or a reporting database fed by an event-driven pipeline.

8. **Q: Your stored procedure's EXPLAIN ANALYZE shows different plans in dev (100 rows) vs prod (10M rows). The prod plan is suboptimal. How do you fix it?**
...
9. **Q: A stored procedure must run in a transaction but internally calls COMMIT (necessary in PostgreSQL maintenance). How does this affect the calling app's transaction?**
...
10. **Q: A stored procedure with ROLLBACK in the EXCEPTION block is migrated to Spring Boot with JPA. The procedure is called via @Procedure and the app's @Transactional also tries to roll back on error. What happens?**
...
3. **What are the benefits of stored procedures?**
...
4. **What are the drawbacks of stored procedures?**
   A: Hard to unit test, difficult to version control, limited debugging, no external service access, weak typing, and logic spread across app + database.

5. **What is SECURITY DEFINER vs SECURITY INVOKER?**
...
6. **When would you use a stored procedure over application code?**
...
7. **How do you call a stored procedure from Spring Data JPA?**
...
8. **What is the performance difference between set-based and row-by-row?**
...
10. **Can stored procedures call external APIs?**
...
- **Version control stored procedures in database migrations** — Store definitions in Flyway/Liquibase migrations, ensuring procedures are versioned and deployed consistently.

- **Use SECURITY DEFINER sparingly** — Grants elevated privileges. Validate inputs and restrict operations. Prefer SECURITY INVOKER with fine-grained table permissions.

- **Test with realistic data volumes** — A procedure fast on 100 rows may be catastrophically slow on 10M rows. Test with production-sized datasets.

- **Add checkpoints to long-running ETL procedures** — Processing 1M rows in a single transaction is risky. Commit in batches and track progress for resumability.
