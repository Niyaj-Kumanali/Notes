...
Reach for query optimization when a query's resource consumption (CPU, I/O, memory) is higher than the workload justifies.

- **API endpoint response time tuning** — Identify and fix N+1 queries generated by Hibernate/JPA using `JOIN FETCH`, `@EntityGraph`, or `@BatchSize`.
  - The most common optimization target in Spring Boot applications.
  - **Avoid when:** the endpoint handles < 10 QPS — optimization effort may not be justified.

- **Batch job optimization** — Replace row-by-row operations with bulk `UPDATE`/`INSERT` to reduce database round trips.
  - A single bulk statement can be 100x faster than individual statements.
  - **Avoid when:** the batch processes fully independent records — consider chunked processing with periodic commits.

- **Reporting dashboard performance** — Add covering indexes, materialized views, and pre-aggregation so million-row queries return in milliseconds.
  - **Avoid when:** the report runs once daily and completes within the SLA — premature optimization adds complexity.

- **Index tuning for complex WHERE clauses** — Analyze query patterns and create composite indexes whose leading column matches the most selective filter.
  - Use `pg_stat_user_indexes` or `sys.dm_db_index_usage_stats` to find unused indexes.
  - **Avoid when:** query patterns are unpredictable — consider full-text search or an external search engine.

- **Reducing database resource utilization** — Rewrite inefficient queries by eliminating function calls in WHERE, reducing sort operations, and projecting only needed columns.
  - Lower CPU and I/O means more headroom for other queries.
  - **Avoid when:** the bottleneck is at the application or network layer, not the database.
