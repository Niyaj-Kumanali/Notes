...
- **LinkedList as Default** — Reaching for `LinkedList` as the general-purpose list because you read "O(1) insert/delete" without considering access patterns.
  - **Why it looks correct:** The asymptotic claim is true and the benchmarks in the JDK documentation show competitive results for small n. In production, iterating 100K elements causes a cache miss on nearly every node dereference — each `Node` is independently allocated on the heap, likely scattered across different memory pages. The GC must traverse every `Node` as a separate heap object, creating ~40 bytes of overhead per element that `ArrayList` avoids entirely. A service processing a message queue with `LinkedList.get(i)` in a loop went from 20ms to 3.2s after the list grew past 50K elements — the O(n) access pattern turned the loop into O(n²). `ArrayList` operations at the end are O(1) amortized with no allocation per element.
...
- **Pre-size collections when size is known** — A `HashMap` growing from default to 100 entries rehashes four times. `new HashMap<>(expectedSize / 0.75f + 1)` eliminates all of that. For `ArrayList`, `new ArrayList<>(expectedSize)` avoids incremental array copying during population. In practice, the cost of not pre-sizing shows up most painfully in latency-sensitive startup paths — a cold-start service loading 50K config entries into an unsized HashMap adds 200ms+ of resize overhead that could be zero with one constructor argument.
