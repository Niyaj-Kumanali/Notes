...
- **LinkedList as Default** — Reaching for `LinkedList` as the general-purpose list because you read "O(1) insert/delete" without considering access patterns. It looks correct because the asymptotic claim is true and the benchmarks in the JDK documentation show competitive results for small n. In production, iterating 100K elements causes a cache miss on nearly every node dereference — each `Node` is independently allocated on the heap, likely scattered across different memory pages. The GC must traverse every `Node` as a separate heap object, creating ~40 bytes of overhead per element that `ArrayList` avoids entirely. A service processing a message queue with `LinkedList.get(i)` in a loop went from 20ms to 3.2s after the list grew past 50K elements — the O(n) access pattern turned the loop into O(n²). `ArrayList` operations at the end are O(1) amortized with no allocation per element.
