...
- **Capturing `this` in Hot Path** — A lambda like `items.forEach(item -> process(item))` inside an instance method captures `this` implicitly because `process()` is an instance method.
  - **Why it looks correct:** The code reads naturally and works fine in tests with small datasets. Each invocation allocates a new lambda object holding a reference to the enclosing object, preventing GC of that object until the lambda is unreachable. In a hot loop called millions of times, this means millions of allocations holding references to the same object. A team discovered this when a batch job processing 5M records OOM'd despite keeping a small working set — lambdas in a `forEach` were capturing the entire Spring `@Service` singleton, preventing its 200MB dependency graph from being collected. The fix: extract to a static method and pass state explicitly: `items.forEach(item -> processHelper(config, item))`.
