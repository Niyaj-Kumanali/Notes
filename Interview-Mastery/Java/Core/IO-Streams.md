...
- **Reading Without Buffering** — Each unbuffered `read()` or `write()` call translates to a system call, and for files processed byte by byte, the overhead of millions of kernel context switches dominates the I/O time.
...
- **TOCTOU Race Condition** — Using `File.exists()` before accessing a file introduces a Time-of-Check-Time-of-Use race condition — the file could be deleted between the check and the open call.
