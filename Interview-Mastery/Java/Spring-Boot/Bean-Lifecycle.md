...
- **Calling `@Transactional` from `@PostConstruct`** — The `@Transactional` does not work because AOP proxies are not yet created during `@PostConstruct`. This *looks correct* because the `@Transactional` annotation is present and the code runs without errors — it silently executes outside a transaction, making this one of the hardest Spring bugs to detect. Use `TransactionTemplate` or restructure to use `@EventListener(ContextRefreshedEvent.class)`.
- **Trying to use uninitialized dependencies in a constructor** — Dependencies are not yet injected during constructor execution. This *looks correct* because constructor parameters appear to be the right place for initialization logic, and the same code works in plain Java where constructor init is standard. The issue is that Spring sets fields/parameters after the constructor body runs. Use `@PostConstruct` for initialization that depends on injected fields, not the constructor itself.
- **Throwing exceptions in `@PostConstruct`** — The application context fails to refresh and the application does not start. This *looks correct* because throwing an exception on validation failure is standard defensive programming — but in `@PostConstruct` it kills the entire application, not just the bean. Handle exceptions gracefully inside init methods with proper logging and fallback logic.
- **Not cleaning up resources** — Failing to implement `@PreDestroy` for closable resources causes memory leaks, exhausted connections, and file handle leaks over time. This *looks correct* because the application runs fine without cleanup during development — the leak only accumulates over hours or days in production, and the application is restarted frequently during development, masking the behavior.
- **Circular init dependencies** — Bean A depends on Bean B and Bean B depends on Bean A, causing `BeanCurrentlyInCreationException`. This *looks correct* because the code models a genuine circular relationship that makes sense at the domain level — the circularity is only a container problem, not a design error in isolation. Use `@Lazy` on one side or restructure to eliminate the cycle.
- **Heavy initialization in singleton beans** — Slows application startup significantly and blocks the entire context refresh. This *looks correct* because loading data at startup ensures fast first requests and follows the "fail fast" principle — the cost only becomes problematic at scale when 50 microservices each take 60 seconds to start. Use `@Lazy` for expensive beans or move initialization to background threads.
- **`final` classes with AOP annotations** — CGLIB cannot create a proxy for final classes, so `@Transactional`, `@Cacheable`, and `@Async` are silently ignored. This *looks correct* because the annotation is present, the method is called, and no error is thrown — the transaction/cache simply never activates, which is extremely hard to debug. Either remove `final` or use an interface for JDK proxying.
