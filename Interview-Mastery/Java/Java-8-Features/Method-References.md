...
**Q: You have a list of `Employee` objects and need to get a sorted list of their department names, case-insensitively. Write this using method references.**
  - `employees.stream().map(Employee::getDepartment).map(String::toLowerCase).distinct().sorted().collect(toList())`
  - Or using a comparator: `employees.stream().map(Employee::getDepartment).distinct().sorted(String::compareToIgnoreCase).collect(toList())`
  - The method references `Employee::getDepartment` (instance method on each element) and `String::toLowerCase` (instance method on the department string) express the mapping steps without variable names.
  - > **Interview follow-up:** If `getDepartment()` returns `null` for some employees, does `String::toLowerCase` throw NPE? How would you filter out null departments within the same pipeline using method references?

**Q: Your codebase has `Function<Order, String> orderToJson = order -> OrderSerializer.toJson(order);`. How would you convert this to a method reference, and what assumption does it make about `toJson`?**
  - `Function<Order, String> orderToJson = OrderSerializer::toJson;` — this assumes `toJson` is a static method on `OrderSerializer` taking one `Order` parameter.
  - If `toJson` were an instance method, the reference would be `orderSerializerInstance::toJson` (requiring a captured instance) or `OrderSerializer::toJson` as `Class::instanceMethod` if `Order` is the receiver and `toJson` takes zero arguments.
  - > **Interview follow-up:** If `OrderSerializer.toJson` is overloaded (one takes `Order`, another takes `Order` and a `FormatOptions`), does the method reference `OrderSerializer::toJson` compile? How does the compiler resolve the overload?

**Q: In a microservice boundary, you have `private final Predicate<Transaction> isDomestic = Transaction::isDomestic;` as a field. The `isDomestic` method is an instance method on `Transaction`. Does this method reference allocate per service instance?**
  - `Transaction::isDomestic` is a `Class::instanceMethod` form — it does not capture any instance, so the method reference is non-capturing and cached as a singleton across all service instances of the class.
  - Even though the field is `private final` per-instance, the `ConstantCallSite` generated by `LambdaMetafactory` is a class-level singleton — all instances of the service class share the same cached method reference.
  - > **Interview follow-up:** If the field were declared as `Predicate<Transaction> isDomestic = t -> t.isDomestic();` (a lambda), would the lambda be a static singleton across all instances or would each service instance create its own lambda object?

**Q: A batch processing job uses `lines.stream().map(Integer::parseInt).collect(toList())` to parse million-line files. `Integer::parseInt` can throw `NumberFormatException`. How do you handle invalid lines within the same stream expression?**
  - Wrap `Integer::parseInt` in a lambda that catches the exception: `lines.stream().map(s -> { try { return Integer.parseInt(s); } catch (NumberFormatException e) { return null; } }).filter(Objects::nonNull).collect(toList())`
  - The method reference alone cannot handle the exception — you must fall back to a lambda for the parse step.
  - For production code, use a utility method: `IntegerParser::parseSafe` that returns `Optional<Integer>` and reference it as a method reference after defining the helper.
  - > **Interview follow-up:** The candidate used `filter(Objects::nonNull)` to remove parse failures. How would you design the utility method so that the pipeline logs each invalid line without breaking the stream's functional purity?
...
**What is the difference between a bound and an unbound method reference?**
  - A bound method reference (`instance::method`) captures a specific instance — the method is called on that captured instance every time. An unbound method reference (`Class::instanceMethod`) receives the instance as the first argument.
  - Example: `System.out::println` is bound (always prints to `System.out`). `String::toUpperCase` is unbound (operates on whatever `String` is passed).
  - Bound references allocate per evaluation of the expression (capturing the instance). Unbound references are non-capturing and cached.

**When would a method reference fail to compile but an equivalent lambda would succeed?**
  - When the method is overloaded — the compiler cannot determine which overload the reference targets.
  - When the method signature uses parameterized types in ways that type inference cannot resolve (e.g., nested generics).
  - When the method is an instance method of a raw type without type parameters to infer.
  - When the target functional interface uses wildcards in a way that constrains parameter types.

**How do you create a method reference to a generic method?**
  - Use the same `Class::method` syntax — the compiler infers the type parameters from the functional interface's target type.
  - `Function<List<String>, Stream<String>> = Collection::stream;` — the compiler infers `<T>` from the `Function` signature.
  - If type inference fails, assign to a typed variable first, or use a lambda with explicit type parameters.

**Can a method reference be assigned to a functional interface with a different parameter count?**
  - Yes — through the `Class::instanceMethod` form where the first parameter becomes the receiver. `BiFunction<String, String, Integer> = String::compareTo;` — two parameters: the receiver `String` and the argument `String`.
  - A static method with no parameters is a `Supplier<T>`. A static method with two parameters is a `BiFunction<T, U, R>`.

**What is the allocation difference between `list.forEach(System.out::println)` and `list.forEach(s -> System.out.println(s))`?**
  - `System.out::println` is a bound instance method reference — it captures the `System.out` reference and allocates one object per evaluation of the expression.
  - `s -> System.out.println(s)` is a capturing lambda — it also captures `System.out` and allocates one object per evaluation.
  - Both are equivalent in allocation. In most code, this allocation occurs once when the expression is evaluated, not per `forEach` iteration. The JIT may inline and scalar-replace both forms.

**How does the `Objects::nonNull` method reference work as a `Predicate`?**
  - `Objects::nonNull` is a `Class::instanceMethod` form where the static method takes one argument and returns `boolean`. The compiler maps it to `Predicate<T>`.
  - It is non-capturing, cached, and used in `stream.filter(Objects::nonNull)` to remove null elements.
  - Similarly, `Objects::isNull`, `String::isEmpty`, and `Collection::isEmpty` are commonly used as predicates in stream pipelines.

**What is a constructor reference with array types?**
  - `int[]::new` is a constructor reference for arrays — it matches `Function<Integer, int[]>` where the argument is the array length.
  - `String[]::new` matches `Function<Integer, String[]>`.
  - Commonly used with `stream.toArray(String[]::new)` instead of `stream.toArray()` which returns `Object[]`.

**How do method references interact with varargs methods?**
  - A method reference to a varargs method works if the functional interface's parameter types match the varargs expansion.
  - Example: `String::format` matches `BiFunction<String, Object[], String>` — the varargs parameter becomes an `Object[]` parameter.
  - If the functional interface expects individual parameters rather than an array, the reference may fail to compile — use a lambda instead.

**What is the relationship between method references and the `@FunctionalInterface` annotation?**
  - The target type must be a functional interface (exactly one abstract method). The `@FunctionalInterface` annotation enforces this at compile time but is not required — any interface with one abstract method can be a method reference target.
  - The compiler checks that the method reference's signature matches the functional interface's abstract method (after accounting for the receiver parameter in `Class::instanceMethod` form).

**Can you use a method reference with a `@Nullable` annotation?**
  - Method references do not inherently propagate `@Nullable` annotations. If the referenced method returns a nullable value, the method reference's type does not reflect this.
  - Use a lambda with explicit null handling to preserve nullable semantics: `s -> s != null ? service.process(s) : null`.

**How does IntelliJ IDEA or Eclipse determine whether to suggest a method reference conversion?**
  - IDEs suggest converting a lambda to a method reference when the lambda body is a single expression that calls a single method and the argument mapping is 1:1 with the lambda parameters.
  - The IDE checks that the method name, parameter count, and return type match the functional interface. It also suggests converting when the lambda body is `return someMethod(x)` or `someMethod(x)` (statement vs expression lambda).
