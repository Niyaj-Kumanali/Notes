...
-   A ORM is an concept which represents the process of mapping java objects to respective relational database tables.
...
-   Maven in java is a software project which is used in build the project, manages the project and dependencies.
...
-   It provides raw fact to the session factory.
-   Session factory uses this raw and produces the session.
    ...
    **Configuration**
    ...
    -   We store the spring information, entity information in an xml file known as `<objectName>.hbm.xml`.
    -   Configuration object is responsible to read and process the above xml files to perform the database related operations.
    ...
    **Session**
    ...
    -   One sessionfactory can generate n number of sessions instances.
...
-   **Types**
    ...
    1. **one to one**
        - one to one mapping represents the process of mapping one object to only one other object
...
-   In ORM, the data fetching is classified into two types:
    ...
    1. **Lazy loading**
        ...
        - **Ex**: @ManyToMany and @OneToMany are considered lazy loading.
    2. **Eager loading**
...
-   There are two types of caching
    ...
    1. **First level cache**
        ...
        - It gets the life when the session is opened and looses the life when session is closed. In other words, the objects that are cached in first level cache or level 1 cache is not visible to first level cache of any other session.
    ...
    2. **Second level cache**
        ...
        - If the data is not present in first level cache memory then the session works for second level cache memory.
...
-   There are three types of queries which helps in performing database operations
    1. **Query**
        ...
        - The object of query interface is created using `createQuery` method.
        - Their are certain methods in query interface, some of them are `public int ExecuteUpdate()`, `public list list()` etc.
        - The `createQuery` method is invoked using session object.
    2. **Native Query**
    3. **Criteria API Query**
        ...
        - We can not use hibernate criteria API for data updation and deletion or any DDL statements.
        - Hibernate criteria API fetches the result from the database using object oriented approach.
        - We have `createCriteria` method which helps in fetching the data from the database based on entity class specified.
...
-   Hibernate is known for implementation of JPA is present in `javax.persistence.package`.
